...
type Props = { saves: SaveVM[]; exploration: ExplorationVM[]; tabbed?: boolean };

function SavesTable({ saves }: { saves: SaveVM[] }) {
  return (
    <Table>
      <tbody>
        {saves.map((s) => (
...
function ExplorationTable({ exploration }: { exploration: ExplorationVM[] }) {
  return (
    <Table>
      <tbody>
        {exploration.map((e) => (
 ...
 * `tabbed` shows them in a two-tab nav (used in the left rail so the column
 * stays above the fold); otherwise they stack with section titles.
 */
export function SavesExploration({ saves, exploration, tabbed = false }: Props) {
  ...
  if (!tabbed) {
    return (
      <section className="rs-section">
        ...
        <SavesTable saves={saves} />
        <SectionTitle hint="1-in-6">Exploration</SectionTitle>
        <ExplorationTable exploration={exploration} />
      </section>
    );
  }
