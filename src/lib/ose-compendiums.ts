...
export const ADVANCED_CLASS_ABILITIES_PACK = "ose-advancedfantasytome.abilities";
export const ADVANCED_RACE_ABILITIES_PACK = "ose-advancedfantasytome.race-abilities";

function classicClassAbilitiesPack(className: string) {
  return game.packs.get(`classicfantasycompendium.abilities-${className.toLowerCase()}`);
}
...
export async function getRaceAbilities(race: string) {
  if (mode === OSE_MODES.ADVANCED) {
    ...
    return items.filter((d) => d?.system?.requirements?.toLowerCase() === race.toLowerCase());
  }
}
...
export async function getClassAbilities(className: string) {
  if (mode === OSE_MODES.ADVANCED) {
    ...
    const filteredItems = items.filter((d) => d?.system?.requirements === className.toLowerCase());
  ...
  if (mode === OSE_MODES.CLASSIC) {
    ...
    if (!abilities) {
      console.error(`[ose ${mode}]`, "No abilities pack found for class:", className);
      return [];
    }
    ...
    return items.filter((d) => classRegexp.test(d?.system?.requirements?.toLowerCase()));
  }
  return [];
}
...
export async function removeClassAbilitiesFromCharacter(actor: OSEActor, className: string) {
  const abilityItems = (actor.items.contents as OseItem[]).filter((i) => i.type === "ability");
  const classRegexp = new RegExp(`${className.toLowerCase()}`, "i");
  const abilitiesToRemove = abilityItems.filter((item: OseItem) => {
